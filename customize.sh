...
在第三行开始编写代码，编写完成后删除下面的参考文章，包括本行；customize意为自定义，这个脚本的作用是在安装模块时执行一些命令；第一行成为shebang，相当于“方言”，它可以为人们和机器指示Shell解释器的类型，建议保留

## 调试代码
Root管理器会内置一个busybox，避免手机厂商的改动影响代码，让脚本始终在可预测的环境中执行。要模拟这个环境，请在终端中执行以下命令（建议复制粘贴避免出错）
```POSIX sh
